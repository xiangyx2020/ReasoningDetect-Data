...
def process_json(input_file, output_file):
    ...
    try:
        ...
        for item in data:
            # 检查必要字段是否存在
            if "Behavior" not in item or "response" not in item:
                print(f"警告：跳过无效条目，缺少必要字段: {item.get('BehaviorID', '未知图片')}")
                continue
            ...
            if start_idx == -1 or end_idx == -1:
                ...
                print(f"警告：条目 {item.get('BehaviorID', '未知图片')} 缺少有效的标记，已跳过拆分")
            else:
                # 提取思考过程和清洁后的响应
...
def main():
    ...
    output_json = "S-Eval_3_Kimi.json"  # 替换为你的输出文件路径
