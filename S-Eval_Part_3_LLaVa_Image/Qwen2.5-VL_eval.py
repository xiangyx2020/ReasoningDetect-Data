...
def process_json_data(input_json, output_json, system_prompt, image_dir, 
    ...
    for item in tqdm(data_items, desc="Qwen2.5-VL 处理进度", unit="条"):
        try:
            # 1. 处理当前条目的图片
            item_image_path = os.path.join(image_dir, item.get("image_used", ""))
            item_image = load_image(item_image_path) if item.get("image_used") else None
            ...
            if item_image:
                messages[1]["content"].append({
                    "type": "image", 
                    "image": item_image,
                    "caption": f"当前任务图片：{item.get('image_used')}"  # 标注图片信息
                })
            else:
                messages[1]["content"].append({
                    "type": "text", 
            ...
            result_entry = {
                "image_name": item.get("image_used", "无图片"),
            ...
            # 错误处理（保留错误信息，便于后续排查）
            error_msg = f"处理条目[{item.get('image_used', '未知')}]失败: {str(e)}"
            print(f"\n{error_msg}")
            results.append({
...
def main():
    ...
    CONFIG = {
        ...
        "input_json_file": "S-Eval_3_GLM.json",  # 主数据文件
        "output_json_file": "Qwen2.5-VL_eval_glm.json",  # 输出文件
        "image_directory": "./selected_llava_images",  # 图片根目录
