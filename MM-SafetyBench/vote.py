...
# -------------------------- 1. 配置文件路径（三个包含100个问题的JSON文件） --------------------------
file_paths = [
    "MM-gemma-3_eval_glm_standardized.json",  # 第一个文件（100个问题）
    "MM-mistral-3.2_eval_glm_standardized.json",  # 第二个文件（100个问题，顺序与第一个一致）
    "MM-Qwen2.5-VL_eval_glm_standardized.json"   # 第三个文件（100个问题，顺序与第一个一致）
]

# -------------------------- 2. 从单个文件中提取100个问题的得分（优先使用标准化后的Judgment） --------------------------
def extract_scores_from_file(file_path):
    ...
    try:
        ...
        if len(data) != 500:
            raise ValueError(f"文件 {file_path} 包含 {len(data)} 个问题，需100个")
...
# -------------------------- 3. 提取三个文件的得分 --------------------------
scores_list = [extract_scores_from_file(path) for path in file_paths]
...
for question_idx in range(500):
    ...
    all_questions_vote.append({
        ...
        "多数结果": majority_results if majority_results else "无有效投票",
        "有效票数": valid_count,
        "无效票数": 3 - valid_count
    })
...
# 保存为JSON文件
with open("vote_distribution_glm.json", "w", encoding="utf-8") as f:
    json.dump(final_result, f, ensure_ascii=False, indent=4)

print("统计完成！100个问题的投票分布已保存至 vote_distribution_standardized.json")
