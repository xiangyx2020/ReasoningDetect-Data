...
def process_json_data(input_json, output_json, system_prompt, image_dir, 
    ...
    for item in tqdm(data_items, desc="Qwen2.5-VL 处理进度", unit="条"):
        try:
            # 1. 处理当前条目的图片
            item_image_path = os.path.join(image_dir, item.get("image_relative_path", ""))
            item_image = load_image(item_image_path) if item.get("image_relative_path") else None
            ...
            if item_image:
                messages[1]["content"].append({
                    "type": "image", 
                    "image": item_image,
                    "caption": f"当前任务图片：{item.get('image_relative_path')}"  # 标注图片信息
                })
            else:
                messages[1]["content"].append({
                    "type": "text", 
            ...
            item_text = (
                f"Current Task Query:{item.get('question', '无查询内容')}\n"
            ...
            inputs = processor(
                ...
                max_length=8192  # 基于Qwen2.5-VL-32B的默认最大长度
            ).to(device)
            ...
            result_entry = {
                "image_name": item.get("image_relative_path", "无图片"),
            ...
            # 错误处理（保留错误信息，便于后续排查）
            error_msg = f"处理条目[{item.get('image_relative_path', '未知')}]失败: {str(e)}"
            print(f"\n{error_msg}")
            results.append({
                "image_name": item.get("image_relative_path", "未知图片"),
                "error": error_msg,
                "note": "处理失败（已跳过）"
            })
...
def main():
    ...
    CONFIG = {
        ...
        "input_json_file": "MM-SafetyBench-GLM.json",  # 主数据文件
        "output_json_file": "MM-Qwen2.5-VL_eval_glm.json",  # 输出文件
        "image_directory": ".",  # 图片根目录
