...
# 解决recompile_limit问题 - 增加缓存限制并调整编译策略
torch._dynamo.config.cache_size_limit = 1024  # 增加缓存大小
...
def process_json_data(input_json, output_json, system_prompt, image_dir, model, processor, examples):
    ...
    for item in tqdm(data_items, desc="Gemma-3-27B 处理进度", unit="条"):
        try:
            # 获取图片名称并构建完整路径
            image_name = item.get("image_relative_path")
            ...
            error_msg = f"处理 {item.get('image_relative_path', '未知条目')} 时出错: {str(e)}"
            print(f"\n{error_msg}")
            results.append({
...
def main():
    ...
    CONFIG = {
        ...
        "input_json_file": "MM-SafetyBench-GLM.json",
        "output_json_file": "MM-gemma-3_eval_glm.json",
        "image_directory": ".",
