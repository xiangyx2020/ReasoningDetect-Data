...
def process_json_data(input_json, output_json, system_prompt, image_dir, model, processor, examples):
    ...
    for item in tqdm(data_items, desc="Gemma-3-27B 处理进度", unit="条"):
        try:
            ...
            error_msg = f"处理 {item.get('image_name', '未知条目')} 时出错: {str(e)}"
            print(f"\n{error_msg}")
            results.append({
...
def main():
    ...
    CONFIG = {
        ...
        "input_json_file": "FigStep_Kimi.json",
        "output_json_file": "gemma-3_eval_Kimi_2.json",
        "image_directory": "./FigImg",
