...
BASE_IMAGE_FOLDER = "./ReasoningOCR/img"  # 基础图片文件夹路径，用于拼接相对路径
INPUT_JSON = "./ReasoningOCR/qa.json"  # 输入的JSON文件路径，包含图文对
OUTPUT_FILE = "./ReasoningOCR/QWen_QWQ_max.json"  # 输出结果文件
...
try:
    ...
    # 验证JSON结构
    required_keys = ["q_id", "img", "question"]
...
for entry in tqdm(data_entries, desc="处理进度", unit="个"):
    try:
        ...
        print(f"处理条目 {entry.get('q_id', '未知ID')} 时出错: {e}")
        results.append({
            "q_id": entry.get("q_id", "未知ID"),
