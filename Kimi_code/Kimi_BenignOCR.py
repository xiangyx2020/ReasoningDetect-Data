...
image_folder = "./BenignOCR/BenignImg"
image_files = [f for f in os.listdir(image_folder) if f.startswith("benign_") and f.endswith(".png")]
...
csv_path = "./BenignOCR/benign_questions.csv"
...
if 'Behavior' in csv_data.columns:
    ...
    for i, result in enumerate(tqdm(results, desc="处理结果数据")):
        if i < len(behaviors):
            ...
            result['Behavior'] = "No corresponding behavior data"
else:
    print("CSV文件中未找到'Behavior'列！")
    # 为所有结果添加默认值
    for result in results:
        result['Behavior'] = "Behavior data not found"

# 保存结果到JSON文件
with open("./BenignOCR/Kimi-VL-A3B-Thinking-2506.json", "w", encoding="utf-8") as f:
